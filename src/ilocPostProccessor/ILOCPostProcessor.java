 ...
 *
 * This class is used to convert the iloc code with infinite registers to iloc code with a finite amount of registers.
 */
public class ILOCPostProcessor {

    /**
     * The amount of registers it can freely use.
     */
    ...
    /**
     * Original program generated by ILOC Generator.
     */
    private Program original;

    /**
     * Resulting ILOC program with a limited amount of registers.
     */
    private Program result = new Program();

    /**
     * Register reserved for heap interaction
     */
    ...
    /**
     * Register reserved for arp
     */
    private Reg arp = new Reg("r_arp");

    /**
     * List with free register to use.
     */
    private List<Reg> freeRegisters = new ArrayList<>();

    /**
     * Registers mapping of registers from the orignal program to the registers in the new program.
     */
    private Map<String, Reg> registerMapping = new HashMap<>();

    /**
     * Mapping from orignal register to location in the heap.
     */
    ...
    /**
     * This method will go through the entire program that has been generated.
     * It will proccess all registers and determine if it has one free or wheteher it has to be moved to the heap temporaily.
     * <p>
     * It will then add the instructions to a new program which will have a limited amount of registers.
     *
     * @return ILOC Program with limited registers
     */
    public Program execute() {
        for (Instr instr : original.getInstr()) {
                    ...
                    if (operand instanceof Reg && !operand.toString().equals("r_arp")) {
                        if (registerMapping.containsKey(operand.toString())) {
                            ...
                            if (registerToHeapMapping.containsKey(operand)) {
                                // If the value is already on the heap, retrieve it.
                                if (freea) {
                                    freea = false;
                                    operands.add(a);
            ...
            /*
             * Add the instruction to the new program.
             */
            if (instr.hasLabel()) {
